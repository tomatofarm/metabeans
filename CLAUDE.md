...
| 문서 | 경로 | 버전 | 용도 |
|------|------|------|------|
| 아키텍처/기술스택 정의서 | `docs/MetaBeans_ESP_프로젝트_아키텍처_기술스택_정의서.md` | v1.1 | 전체 아키텍처, 폴더 구조, 기술 선택 근거, Mock→API 전환 패턴, 자동제어 동작 규칙 |
| 데이터구조 정의서 | `docs/MetaBeans_ESP_데이터구조_정의서_v3_0.md` | v3.1 | DB 스키마(테이블/컬럼), 엔티티 관계, MQTT 메시지 구조, 비즈니스 규칙 |
| REST API 설계서 | `docs/MetaBeans_ESP_REST_API_엔드포인트_설계서_v1_0.md` | v1.1 | 83개 API 엔드포인트 명세, 요청/응답 구조, 에러 코드 |
| MQTT 프로토콜 설계서 | `docs/MetaBeans_ESP_MQTT_통신_프로토콜_설계서_v1_0.md` | v2.0 | MQTT 토픽, 페이로드 파싱, 알람 판정, 30초 타임아웃, 제어 ACK, config 설정 |
| 최종 피드백 (PDF) | `docs/ESP_관리툴_최종피드백_260212.pdf` | - | UI 수정사항, 화면별 피드백 (최우선 반영) |
| MQTT Payload 규격 | `docs/MQTT_Payload_규격_260213.pdf` | 260213 | 최신 센서 필드(fan_mode, damper_mode), 자동제어 action 추가 |
...
| damper | float | % | 댐퍼 개도율 (flo-OAC Damper_FB, 0-100) |
...
## v3.0/v3.1 주요 변경사항 (피드백 + MQTT 260213 반영)
