...
import { Card, DatePicker, Select, Table, Space, Spin, Empty, Typography } from 'antd';
...
// 컨트롤러별 차트 색상
const LINE_COLORS = ['#1890ff', '#52c41a', '#faad14', '#ff4d4f', '#722ed1', '#13c2c2'];
...
const WARNING_ZONES: Partial<Record<SensorField, { min: number; max: number; color: string }[]>> = {
  ...
  ppSpark: [
    { min: 30, max: 70, color: 'rgba(250,173,20,0.08)' },
    { min: 70, max: 100, color: 'rgba(255,77,79,0.08)' },
  ],
  pm25: [
...
export default function SensorHistoryTab({ equipmentId }: SensorHistoryTabProps) {
  ...
  return (
    <div>
      ...
      {/* 테이블 */}
      <Card size="small" title="센서 데이터 상세">
        <Table
          columns={columns}
