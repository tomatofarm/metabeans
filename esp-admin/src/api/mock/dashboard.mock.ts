...
import { mockDelay, wrapResponse } from './common.mock';
...
function generateControllerSensorData(
  ...
  return {
    ...
    oilLevel: randomFloat(SENSOR_RANGES.oilLevel.min, SENSOR_RANGES.oilLevel.max, SENSOR_RANGES.oilLevel.decimals),
...
function generateGatewaySensorData(gatewayId: number): GatewaySensorData {
  return {
    gatewayId,
    timestamp: Math.floor(Date.now() / 1000),
    pm10: randomFloat(10, 60, 1),
    pm25: randomFloat(5, 30, 1),
    pm40: randomFloat(15, 70, 1),
    pm100: randomFloat(20, 90, 1),
      ...
      controllers: [
        {
          ...
          connectionStatus: 'ONLINE',
          status: 'yellow',
          sensorData: generateControllerSensorData('ctrl-003', { diffPressure: 42.5, ppSpark: 45 }),
          lastSeenAt: new Date().toISOString(),
        },
      ],
